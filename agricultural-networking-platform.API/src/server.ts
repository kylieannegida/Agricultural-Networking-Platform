import http from "http";
import express from "express";
import mongoose from "mongoose";
import swaggerUi from "swagger-ui-express";

import "./config/logging";
import { DEVELOPMENT, mongo, server } from "./config/config";

import "reflect-metadata";

import { corsHandler } from "./middleware/corsHandler";
import { loggingHandler } from "./middleware/loggingHandler";
import { routeNotFound } from "./middleware/routeNotFound";
import { specs } from "./config/swagger";
import BlockedUsersRoutes from "./routes/BlockedUsersRoutes";
import CommentReactionRoutes from "./routes/CommentReactionRoutes";
import CommentRoutes from "./routes/CommentRoutes";
import CommunityEventAttendeeRoutes from "./routes/CommunityEventAttendeeRoutes";
import CommunityEventRoutes from "./routes/CommunityEventRoutes";
import CommunityMemberRoutes from "./routes/CommunityMemberRoutes";
import CommunityPostRoutes from "./routes/CommunityPostRoutes";
import CommunityReportRoutes from "./routes/CommunityReportRoutes";
import CommunityRoutes from "./routes/CommunityRoutes";
import FarmerRoutes from "./routes/FarmerRoutes";
import FriendshipRoutes from "./routes/FriendshipRoutes";
import LikeRoutes from "./routes/LikeRoutes";
import PostReactionRoutes from "./routes/PostReactionRoutes";
import PostReportRoutes from "./routes/PostReportRoutes";
import PostRoutes from "./routes/PostRoutes";
import PostTagRoutes from "./routes/PostTagRoutes";
import ShareRoutes from "./routes/ShareRoutes";
import UserSettingsRoutes from "./routes/UserSettingsRoutes";
import CommentReportRoutes from "./routes/CommentReportRoutes";
import authRoutes from "./routes/authRoutes";
import userRoutes from "./routes/userRoutes";

export const application = express();
export let httpServer: ReturnType<typeof http.createServer>;

export const Main = async () => {
  logging.log("----------------------------------------");
  logging.log("Initializing API");
  logging.log("----------------------------------------");
  application.use(express.urlencoded({ extended: true }));
  application.use(express.json());

  logging.log("----------------------------------------");
  logging.log("Swagger UI");
  logging.log("----------------------------------------");
  application.use(
    "/api/docs",
    swaggerUi.serve,
    swaggerUi.setup(specs, {
      explorer: true,
      customCss: ".swagger-ui .topbar { display: none }",
      customSiteTitle: "Your API Documentation",
    })
  );

  logging.log("----------------------------------------");
  logging.log("Connect to Mongo");
  logging.log("----------------------------------------");
  try {
    const connection = await mongoose.connect(
      mongo.MONGO_CONNECTION,
      mongo.MONGO_OPTIONS
    );
    logging.log("----------------------------------------");
    logging.log("Connected to Mongo: ", connection.version);
    logging.log("----------------------------------------");
  } catch (error) {
    logging.log("----------------------------------------");
    logging.error(error);
    logging.error("Unable to connect to Mongo");
    logging.log("----------------------------------------");
  }

  logging.log("----------------------------------------");
  logging.log("Logging & Configuration");
  logging.log("----------------------------------------");
  application.use(loggingHandler);
  application.use(corsHandler);

  logging.log("----------------------------------------");
  logging.log("Define Controller Routing");
  logging.log("----------------------------------------");
  application.use(authRoutes);
  application.use(userRoutes);
  application.use(CommentReactionRoutes);
  application.use(CommentReportRoutes);
  application.use(CommentRoutes);
  application.use(CommunityEventAttendeeRoutes);
  application.use(CommunityEventRoutes);
  application.use(CommunityMemberRoutes);
  application.use(CommunityPostRoutes);
  application.use(CommunityReportRoutes);
  application.use(CommunityRoutes);
  application.use(FarmerRoutes);
  application.use(FriendshipRoutes);
  application.use(LikeRoutes);
  application.use(LikeRoutes);
  application.use(PostReactionRoutes);
  application.use(PostReportRoutes);
  application.use(PostRoutes);
  application.use(PostTagRoutes);
  application.use(ShareRoutes);
  application.use(UserSettingsRoutes);
  application.use(BlockedUsersRoutes);

  logging.log("----------------------------------------");
  logging.log("Define Routing Error");
  logging.log("----------------------------------------");
  application.use(routeNotFound);

  logging.log("----------------------------------------");
  logging.log("Starting Server");
  logging.log("----------------------------------------");
  httpServer = http.createServer(application);
  httpServer.listen(server.SERVER_PORT, () => {
    logging.log("----------------------------------------");
    logging.log(
      `Server started on ${server.SERVER_HOSTNAME}:${server.SERVER_PORT}`
    );
    logging.log("----------------------------------------");
  });
};

export const Shutdown = (callback: any) =>
  httpServer && httpServer.close(callback);

Main();
